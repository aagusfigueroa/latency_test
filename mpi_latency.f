...
C     status                  =  MPI receive routine parameter                                                                                               
      ...
      real, allocatable :: timesf(:), nztimes(:,:)
...
c      character msg
      ...
      allocate(nztimes(numtasks*(numtasks-1)/2,3))
...
40     continue
       do 50 i=1,numtasks*(numtasks-1)/2
       write(iunit,*)(nztimes(i,j),j=1,3),i
       if (i.eq.1) then
c      nothing happend
...
50     continue
       sumtotal=sum(nztimes(:,3))/(numtasks*(numtasks-1)/2)
